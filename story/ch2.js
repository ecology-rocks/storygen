...
module.exports = {
    ...
    title: pep.choice("Trouble in Paradise", "Heart Of Stone", "The Liar's Touch"),
    text: pep.seq(html.p('o'),
        //p1
        pep.choice(
            pep.seq('"Is ', names.eddie, ' ', dict.insane, '?" '),
            pep.seq('"Do you think this whole ', names.eddie, ' stuff is ', dict.legit, '?" '),
            pep.seq('"What do you think about our ', dict.impending, ' ', dict.doom, '?" '),
        ), pep.choice(
            pep.seq(names.jerry, ' ', dict.asked, ', ', dict.gnawing, ' ', pronouns.their(names.jerry0),
                ' way through a piece of ', dict.overcooked, ' meat. '),
            pep.seq(names.jerry, ' ', dict.wondered, ' as ', pronouns.they(names.jerry0), ' ', dict.gnawed,
                ' the final bites of ', pronouns.their(names.jerry0), ' meal. '),
        ...
        pep.choice(
            ...
            pep.seq('"You ', dict.always, ' ask the ', dict.tough, ' questions, ', names.jerry, '." '),
        ), pep.choice(
            pep.seq(names.reed, ' ', dict.wondered, ', ', dict.pushing, ' the tray away. '),
            pep.seq(names.reed, ' pushed ', pronouns.their(names.reed0), ' ', dict.tray, ' away and ',
                dict.crossed, ' ', pronouns.their(names.reed0), ' arms. '),
            pep.seq(names.reed, ' ', dict.grinned, ' a ', dict.toothy, ' ', dict.grin, ' at ', names.jerry,
                '. '),
        ), pep.choice(
            pep.seq('"But is anyone going to stop ', pronouns.them(names.eddie0), '?" '),
            pep.seq(names.reed, ' was ', myPep.aAn(dict.intimidating), ' ', dict.sight,
                ', even with ', dict.grape, ' ', dict.jelly, ' ', dict.drippingFrom, ' ', pronouns.their(
                    names.reed0), ' ', dict.chin, '. '),
            pep.seq('"There’s ', dict.nothing, ' we can do about it." '),
        ), html.pco,
        //p3       
        pep.choice(pep.seq('That was the ', dict.issue, ', after all. '),
            pep.seq('It was the ', dict.issue, ' on every ', world.brethren(names.realm),
                '\'s mind. '),
            pep.seq('It was what kept ', world.brethrens(names.realm), ' up at night. '),
        ), pep.choice(
            pep.seq('Would anyone ', dict.stop, ' ', names.eddie, '’s ', dict.planned, ' ', dict.rebellion,
                ' against the ', world.boss(names.realm), '? '),
            pep.seq('Would ', names.eddie, ' ', dict.retreat, ' before it was too late? '),
            pep.seq('Would anyone put ', names.eddie, ' in ', pronouns.their(names.eddie0),
                ' place? '),
        ), pep.choice(
            pep.seq('Or were ', world.brethrens(names.realm), ' really that ', dict.frustrated,
                ' with the current ', world.boss(names.realm), '\'s style of rule? '),
            pep.seq('Or were ', world.brethrens(names.realm), ' really that ', dict.apathetic, ' to ',
                dict.impending, ' ', dict.war, '?'),
            pep.seq('Was it even possible? '),
        ), pep.choice(
            pep.seq('It was ', dict.tough, ' to ', dict.ascertain, ', because there was ', dict.always,
                ' a level of ', dict.trouble, ' in ', world.realm(names.realm), ' -- but ', dict.somehow,
                ', it ', dict.seemed, ' like this time was ', dict.serious, '. '),
            pep.seq('There had ', dict.always, ' been a level of ', dict.trouble, ' in ', world.realm(
                names.realm), ', but this time seemed more ', dict.serious, '. '),
            pep.seq('The entire ', dict.realm, ' of ', world.realm(names.realm), ' ', dict.teetered,
                ' on ', dict.uncertainty, ' and ', dict.impending, ' ', dict.war, '. '),
        ), pep.choice(
            pep.seq('Like they ', dict.could, ' ', dict.actually, ' mean it.'),
            pep.seq('Like it ', dict.could, ' ', dict.actually, ' ', dict.happen, '. '),
            pep.seq('This ', dict.time, ' was ', dict.dangerously, ' ', dict.different, '. '),
        ), html.pco,
        //p4     
        pep.choice(
            pep.seq(names.jerry, ' ', dict.savored, ' ', pronouns.their(names.jerry0), ' ', dict.last,
                ' ', dict.morsel, '. '),
            pep.seq(names.jerry, ' was ', dict.suddenly, ' no longer ', dict.hungry, '. '),
            pep.seq('The ', dict.food, ' had ', dict.lost, ' its ', dict.appeal, '. '),
        ), pep.choice(
            pep.seq('"I feel like this ', dict.impending, ' ', dict.war, ' spells trouble. '),
            pep.seq('"I hate this ', pep.opt(pep.seq(dict.steaming, ' ', dict.pile, ' of ')), dict.shit,
                '. '),
            pep.seq('"I ', pep.choice("hope", "wish"), ' they would all ', dict.selfImmolate, '. '),
        ), pep.choice(
            pep.seq('We\'re all going to be ', dict.dragged, ' into this ', dict.steaming, ' ', dict.pile,
                ' of ', pep.choice(dict.dog, dict.animalList), '-', dict.shit, ' ', dict.war, '. '),
            pep.seq('You know who ', dict.dies, ' when ', dict.rulers, ' fight? It\'s us, ', dict.damnIt,
                '.'),
            pep.seq('Who cares who the ', world.boss(names.realm), ' is? '),
        ), pep.choice(
            pep.seq('I don\'t ', pep.opt('particularly '), 'care who ', dict.wipes, ' their ', dict.ass,
                ' at the top of the food chain -- it doesn’t really ', dict.affect,
                ' me from day to day." '),
            pep.seq('The ', dict.rulers, ' don\'t ', dict.affect, ' our ', dict.shitty,
                ' lives one way or another." '),
            pep.seq('I could go my ', dict.whole, ' ', dict.life, ' not ', dict.fretting, ' about ',
                dict.rulers, ', but then they ', dict.drag, ' us into their ', dict.war, '." '),
        ), html.pco,
        //p5    
        pep.choice(
            pep.seq(names.reed, ' ', dict.barked, ' a ', dict.harsh, ' ', dict.laugh, pep.opt(
                ' in response'), '. '),
            pep.seq(names.reed, ' ', myPep.edIng(dict.cackling), ' at ', names.jerry, '\'s ', dict.bluntness,
                '. '),
            pep.seq(names.reed, ' nodded ', dict.heartily, ' in ', dict.agreement, '. '),
        ), pep.choice(
            pep.seq('"The ', dict.truest, ' thing that has ever been ', dict.spoken, '. '),
            ...
            pep.seq('Come on -- let\'s go before ', names.kaden, ' gets ', dict.raging, ' ', dict.angry,
                '." '),
            pep.seq('Let\'s get back before ', names.kaden, ' notices." '),
            pep.seq('At least it\'s only a ', dict.few, ' hours to midnight." '),
        ), html.pco,
        //p6     
        pep.choice(
        ...
        pep.choice(
            ...
            pep.seq(pep.capitalize(pronouns.their(names.jerry0)), ' stomach was letting ', pronouns.them(
                    names.jerry0), ' know that this may be ', pronouns.their(names.jerry0),
                ' worst fear come to life. '),
            pep.seq(names.jerry, ' felt ', pronouns.their(names.jerry0), ' courage waver. '),
            pep.seq(names.jerry, ' was suddenly hyper aware of ', pronouns.their(names.jerry0),
                ' sweaty palms and inadequate stature. '),
        ), html.pco,
        //p11   
        pep.choice(
        ...
        pep.choice(
            ...
            pep.seq('"You mean about the rebellion? '),
            pep.seq('"Another upstart? '),
        ), pep.choice(
            pep.seq('Not much. '),
            pep.seq('That\'s news to me. '),
            pep.seq('I assumed it was just talk. '),
        ), pep.choice(
            pep.seq('Why?" '),
            pep.seq('Is it serious?" '),
            pep.seq('Do you think we should worry?" '),
        ), html.pco,
        //p13   
        pep.choice(
            ...
            pep.seq(names.kaden,
                ' trailed off as if trying to find a delicate way to state something offensive. '),
            pep.seq(names.kaden, ' paused as if gathering courage to say something. '),
            pep.seq(names.kaden, ' took a breath. '),
        ), pep.choice(
            pep.seq('Though ', names.kaden, ' had never been the polite sort. '),
            pep.seq(names.jerry, ' wondered what ', names.kaden, ' was about to drop on ', pronouns.them(
                names.jerry0), '. '),
            ...
            pep.seq(pep.capitalize(pronouns.they(names.kaden0)), ' settled on cutting to the chase. '),
            pep.seq(names.kaden, ' continued. '),
            pep.seq(names.kaden, ' grinned sheepishly. '),
        ), pep.choice(
            pep.seq('If this comes to blows, you know we’re on the front lines. '),
            pep.seq('"You know we\'re the first line of defense, right?  '),
            ...
            pep.seq('You haven\'t exactly prove yourself in the past." '),
        ...
        pep.choice(
            ...
            pep.seq(names.jerry, ' cut ', names.kaden, ' off before ', pronouns.they(names.kaden0),
                ' could reiterate what ', names.jerry, ' had failed to do. '),
            pep.seq(names.jerry, ' reassured the commander. '),
            pep.seq(names.jerry, ' blushed with embarrassment at being called out. '),
        ), pep.choice(
            pep.seq(names.jerry, ' felt the falseness of the words as they left ', pronouns.their(
                names.jerry0), ' mouth. '),
            pep.seq(names.jerry, ' knew as soon as the words came out, that they were a lie.'),
        ...
        pep.choice(
            ...
            pep.seq('I know you\'ve got those deep thoughts and all... '),
            ...
            pep.seq('You put this rebellion down if it comes to your front door. '),
            pep.seq('Those rebels deserve everything they\'ve got coming to them. '),
        ), pep.choice(
            pep.seq('You hear me? '),
        ...
        pep.choice(
            ...
            pep.seq(names.jerry, '\'s palms were itchy and a sheen of sweat was breaking out across ',
                pronouns.their(names.jerry0), ' skin. '),
            pep.seq(names.jerry, ' was more than ready to get out of this conversation. '),
            pep.seq(names.jerry, '\'s mouth was full of ash. '),
        ), pep.choice(
            pep.seq(pep.capitalize(pronouns.they(names.jerry0)), ' wondered if ', names.kaden,
                ' noticed. '),
            pep.seq('"Thank you for bringing this to my attention." '),
            pep.seq(names.jerry, ' felt their knees shaking just a little bit. '),
        ), html.pco,
        //p17           
        pep.choice(
            ...
            pep.seq('"And you will die either doing them or refusing them. '),
        ...
        pep.choice(
            ...
            pep.seq('Did the commander just say that ', pronouns.they(names.jerry0),
                ' would die in battle? '),
            pep.seq('Did the commander just threaten ', pronouns.them(names.jerry), '? '),
        ...
        pep.choice(
            ...
            pep.seq(names.jerry, ' mustered all of the courage ', pronouns.they(names.jerry0),
                ' could to make it sound convincing. '),
            pep.seq(names.jerry, ' knew that names.kaden was dead serious. '),
            pep.seq(names.jerry, ' felt the sweats come in ', pronouns.their(names.jerry), ' fear. '),
        ), pep.choice(
            pep.seq('"I wouldn\'t abandon my fellow soldiers in the field of war." '),
            pep.seq('"I would never do anything to endanger my squad." '),
            pep.seq('"I would give my life for our ', world.boss(names.realm), '." '),
        ), html.pco,
        //p21   
        pep.choice(
            pep.seq(names.kaden, '’s searching gaze seemed to invade ', names.jerry, '’s soul. '),
            pep.seq(names.kaden, ' stared menacingly at ', names.jerry, '. '),
            pep.seq('There was a moment of silence as ', names.kaden, ' evaluated the truth of ',
                names.jerry, '\'s statement. '),
        ), pep.choice(
            pep.seq('But they could not -- thank the dogs -- detect the lie in it. '),
            pep.seq('Thankfully, ', names.kaden, ' seemed blissfully unaware of ', names.jerry,
                '\'s nervousness. '),
            ...
            pep.seq(names.kaden, ' seemed satisfied, though, and smiled. '),
        ), html.pco,
        //p22         
        pep.choice(
        ...
        //p23            
        pep.choice(
            pep.seq(names.jerry, ' walked past the command desk and up the curling stairs to ',
                pronouns.their(names.jerry0), ' post. '),
            pep.seq(names.jerry, ' carried onwards up the stairs. '),
            pep.seq(names.jerry, ' left the command post and headed upstairs to ', pronouns.their(
            ...
            pep.seq('Was ', names.eddie, ' really about to take down the ', world.boss(names.realm),
                '? '),
            pep.seq('Could war be on the horizon? '),
        ), pep.choice(
            pep.seq('How could they get out of it? '),
            pep.seq('How would ', names.jerry, ' find a way to avoid fighting? '),
            pep.seq('Would this particular town be safe from the horrors of war? '),
        ), html.pco,
        //p24           
        pep.choice(
            pep.seq('Peering across the dusty landscape surrounding the compound, ', names.jerry,
                ' idly wondered if ', pronouns.they(names.jerry0), ' could fling ', pronouns.themself(
                    names.jerry0), ' from the ramparts. '),
            pep.seq(names.jerry, ' contemplated suicide by rampart as ', pronouns.they(names.jerry),
                ' scanned the dusty landscape. '),
            pep.seq(names.jerry, ' briefly wondered if death was better than war. '),
        ), pep.choice(
            pep.seq('After all, the hospitalized could not fight. '),
            pep.seq('Even if ', pronouns.they(names.jerry), ' didn\'t die, ', names.jerry,
                ' could at least avoid the fighting. '),
            pep.seq('But what if ', pronouns.they(names.jerry), ' failed even at suicide? '),
        ), pep.choice(
            ...
            pep.seq('Or maybe invading forces would not look kindly at prisoners of war. '),
            pep.seq('Likely, ', pronouns.they(names.jerry),
                ' would be killed anyway by invading forces. '),
        ), pep.choice(
            pep.seq(world.brethrens(names.realm), ' were not kind in civil war. '),
            pep.seq(world.brethrens(names.realm), ' did not fuck around in times of war. '),
            pep.seq(names.jerry, ' would not last long in the hands of ', pronouns.their(names.jerry),
                ' enemies. '),
        ), pep.choice(
            pep.seq('There were no rules to follow. '),
            pep.seq('There was no Geneva Convention in ', world.realm(names.realm), '. '),
            pep.seq('There were no rules in civil war. '),
        ), pep.choice(
            pep.seq('Dead could be buried -- wounded needed care. '),
            pep.seq('And the dead were much easier to deal with than the wounded. '),
            pep.seq('Prisoners were more expensive than mass graves. '),
        ), html.pco,
        //p25         
        pep.choice(
            pep.seq('Looking down the thirty foot drop though, ', names.jerry,
                ' decided that perhaps suicide was not the best way. '),
            pep.seq('Looking down into the chasm, ', names.jerry,
                ' decided that suicide was not for ', pronouns.them(names.jerry), '. '),
            pep.seq(names.jerry, ' shied away from thoughts of suicide. '),
        ), pep.choice(
            pep.seq('There had to be another. '),
            pep.seq('There had to be a way to stay out of war that didn\'t result in pain or death. '),
            pep.seq('Like momma always said, there\'s a better way to do things. '),
        ), pep.choice(
            pep.seq('Could ', pronouns.they(names.jerry0),
                ' request a transfer to the medical or other nonviolent units? '),
            pep.seq('Maybe ', pronouns.they(names.jerry), ' could request a transfer. '),
            pep.seq('Maybe ', pronouns.they(names.jerry), ' could request a promotion. '),
        ), pep.choice(
            pep.seq(pep.capitalize(pronouns.they(names.jerry0)), ' could -- but ', pronouns.they(
                    names.jerry0), ' would be laughed at, for ', pronouns.they(names.jerry0),
        ...
        pep.choice(
            pep.seq(pep.capitalize(pronouns.they(names.jerry0)),
                ' could join the revolution, although then ', pronouns.they(names.jerry0),
                '’d just be fighting for the otherside, and ', names.jerry,
                ' doubted that they were kinder. '),
            pep.seq(names.jerry, ' contemplated joining the rebellion. '),
            pep.seq(names.jerry, ' wondered if joining the rebellion would somehow be safer. '),
        ), pep.choice(
            pep.seq('What about an extended vacation, perhaps to the mortal realm? '),
            ...
            pep.seq('Either could help ', names.jerry, ' forego battle, although ', names.jerry,
            ...
            pep.seq(names.jerry, ' knew that these were just fantasies. '),
        ), pep.choice(
            pep.seq('You know, pending civil war and all. '),
            pep.seq('Pending civil war really narrowed the options for escape. '),
        ...
        pep.choice(
            ...
            pep.seq('"Did you hear the mortal world\'s been cut off from us?" '),
        ), pep.choice(
            pep.seq(names.dorian, '\'s voice came from behind ', names.jerry, ', startling them.'),
            pep.seq(names.dorian, ' spoke behind ', names.jerry, '. '),
            pep.seq(names.dorian, ' blew cigar smoke in ', names.jerry, '\'s face as ', pronouns.they(
        ...
        pep.choice(
            ...
            pep.seq(names.jerry, ' couldn\'t tell if ', names.dorian, ' was kidding. '),
        ), html.pco,
        //p29         
        pep.choice(
            pep.seq('"Yeah, no one is supposed to leave or enter until this whole mess with ', names.eddie,
                ' is done." '),
            pep.seq('"They\'ve got the portals under lock and key until ', names.eddie,
                ' is dealt with." '),
            pep.seq('"Yeah, portals are closed and there\'s a bounty on ', names.eddie, ', too." '),
        ), pep.choice(
            pep.seq(names.dorian, ' huffed on a cigarette, releasing a long trail of white smoke. '),
            pep.seq(names.dorian, ' took another drag of ', pronouns.their(names.dorian), ' cigar. '),
        ...
        pep.choice(
            ...
            pep.seq(names.jerry, ' felt the unease grow inside ', pronouns.them(names.jerry), '. '),
        ), pep.choice(
        ...
        pep.choice(
            ...
            pep.seq('I can\'t believe the ', world.boss(names.realm), ' let it get this far. '),
        ), pep.choice(
            pep.seq('I\'d hate to get blood on my brand new dagger." '),
            pep.seq('I didn\'t want to blood my new dagger on ', world.brethrens(names.realm), '." '),
            pep.seq('I didn\'t actually want to use this dagger." '),
        ), pep.choice(
            pep.seq(names.dorian, ' hefted a shiny blade from ', pronouns.their(names.dorian0),
                ' waist belt. '),
            pep.seq(names.dorian, ' displayed a bright new blade from ', pronouns.their(names.dorian),
                ' belt. '),
            pep.seq(names.dorian, ' fingered a blade on ', pronouns.their(names.dorian), ' belt. '),
        ), pep.choice(
            pep.seq('"But I guess if traitor blood is the only way to whet it, count me in." '),
            pep.seq('"I hope it does me good though, when the time comes." '),
        ...
        pep.choice(
            ...
            pep.seq(pep.capitalize(pronouns.they(names.jerry)), ' decided on a smile. '),
        ...
        pep.choice(
            ...
            pep.seq('"', names.dorian, ' says you\'re our first and biggest coward." '),
            pep.seq('"', names.dorian, ' is saying you\'ll break rank and run when the time comes." '),
        ), pep.choice(
        ...
        pep.choice(
            ...
            pep.seq(names.jerry, ' spoke without a waver in ', pronouns.their(names.jerry),
                ' voice. '),
            ...
            pep.seq('But ', names.dorian, ' had an unblooded knife, and ', names.jerry,
                ' wasn\'t about to be its maiden voyage. '),
            pep.seq('But ', names.dorian, ' had a knife, so honesty wasn\'t an option. '),
            pep.seq('Anything to keep ', pronouns.them(names.jerry), ' alive another day. '),
        ), html.pco,
        //p35          
        pep.choice(
            ...
            pep.seq(pep.capitalize(pronouns.they(names.dorian)), ' had not expected such furor from ',
                names.eddie, '. '),
            pep.seq(names.dorian, ' was utterly taken aback by ', names.jerry, '\'s boldness. '),
        ), pep.choice(
            pep.seq(names.dorian, ' slapped ', names.jerry, ' on the back. '),
