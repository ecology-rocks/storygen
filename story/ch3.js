...
let aText = {
    ...
    p1as1a: pep.seq('The moon hung low in the sky as ', names.jerry,
        ' walked across the courtyard and into the administrative building that led to freedom. '),
    p1as1b: pep.seq(names.jerry,
        ' crunched through the courtyard towards the ominous administrative building. '),
    p1as1c: pep.seq('The moon hung ominously over the administration building as ', names.jerry, ' made ',
        pronouns.their(names.jerry0), ' way home. '),

    p1as2a: pep.seq('It had been a long shift, but it was over and ', names.jerry,
        ' could not wait to crawl into bed. '),
    p1as2b: pep.seq(names.jerry, ' was thankful that the arduous shift was over. '),
    p1as2c: pep.seq(names.jerry, '\'s thoughts drifted as ', pronouns.they(names.jerry0),
    ...
    p2as1b: pep.seq('The door opened with a creak as names.jerry stepped into the dimly lit hallway. '),
    p2as1c: pep.seq(names.jerry, ' always felt like ', pronouns.they(names.jerry0),
    ...
    p2as2b: pep.seq('It was amazing to ', names.jerry,
    ...
    p2as3a: pep.seq('The preternatural quiet was harshly interrupted by ', names.jerry,
        '\'s combat books clunking upon the stone floor. '),
    p2as3b: pep.seq(names.jerry,
        '\'s boots disrupted the quiet hallway as they clattered against the stone. '),
    p2as3c: pep.seq('The hallway was quiet, punctuated only by ', names.jerry,
        '\'s heavy breath and boots upon the floor. '),

    //p3

    p3as1a: pep.seq(names.jerry, ' turned a corner and found ', pronouns.themself(names.jerry0),
        ' in the midst of chaos. '),
    p3as1b: pep.seq(names.jerry, ' abruptly found ', pronouns.themself(names.jerry0),
        ' surrounded by activity as ', pronouns.they(names.jerry0), ' rounded the corner. '),
    p3as1c: pep.seq(names.jerry, ' heard voices just around the bend. '),

    p3as2a: pep.seq('The lights were much brighter here, and orange cones and caution tape blocked ',
    ...
    p3as2c: pep.seq(names.jerry,
        ' saw several folks on the other side of orange cones and caution tape. '),

    p3as3a: pep.seq('A fellow guard, weapon loose in hands, nodded abruptly at ', names.jerry, '. '),
    p3as3b: pep.seq('The guard closest to ', names.jerry, ' grunted. '),
    p3as3c: pep.seq(names.jerry, ' saw the nearest guard eyeing ', pronouns.them(names.jerry0), '. '),
    ...
    p5as2a: pep.seq(names.jerry,
        ' offered, surveying past the soldier to the fresh splats of blood on the wall. '),
    p5as2b: pep.seq('names.jerry wondered what the story was. '),
    p5as2c: pep.seq('names.jerry offered absentmindedly, immediately regretting the decision. '),

    p5as3a: pep.seq(world.brethrens(names.realm), ' in hazmat suits were hard at work. '),
    p5as3b: pep.seq('The hazmats buzzed around behind the guard like busy little bees. '),
    p5as3c: pep.seq('Hazmatted ', world.brethrens(names.realm), ' were staring. '),
    ...
    p6as4a: pep.seq('The soldier took a breath and ', names.jerry,
        ' realized belatedly that there was fresh blood on the sword that the guard held. '),
    p6as4b: pep.seq(''),
    p6as4c: pep.seq(''),

    p6as5a: pep.seq(pep.capitalize(pronouns.their(names.jerry0)), ' stomach turned. '),
    p6as5b: pep.seq(''),
    p6as5c: pep.seq(''),
    ...
    p8as2a: pep.seq(names.jerry, ' did a small salute and turned to walk away. '),
    p8as2b: pep.seq(''),
    p8as2c: pep.seq(''),
    ...
    p9as2a: pep.seq('The soldier shouted as ', names.jerry, ' departed. '),
    p9as2b: pep.seq(''),
    p9as2c: pep.seq(''),

    p9as3a: pep.seq(pep.capitalize(pronouns.they(names.jerry0)),
        ' were unsure what to make of the scene, but bed still called ', pronouns.their(names.jerry0),
        ' name. '),
    p9as3b: pep.seq(''),
    p9as3c: pep.seq(''),
    ...
    p10as2a: pep.seq(names.jerry, ' slowed ', pronouns.their(names.jerry0), ' pace as ', pronouns.they(
            names.jerry0),
    ...
    p10as3a: pep.seq(pep.capitalize(pronouns.they(names.jerry0)), ' unsheathed ', pronouns.their(names.jerry0),
        ' own dagger, gripping it with a white-knuckled ferocity.'),
    p10as3b: pep.seq(''),
    p10as3c: pep.seq(''),

    //p11
    p11as1a: pep.seq(names.jerry, ' peered around the corner, filled with apprehension. '),
    p11as1b: pep.seq(''),
    p11as1c: pep.seq(''),
    ...
    p11as4a: pep.seq('Cautiously, ', names.jerry,
        ' stepped across the threshold, dagger held at the ready. '),
    p11as4b: pep.seq(''),
    p11as4c: pep.seq(''),

    p11as5a: pep.seq(pep.capitalize(pronouns.they(names.jerry0)), ' flipped ', pronouns.their(names.jerry0),
    ...
    p12as4a: pep.seq('Nothing seemed out of the ordinary, other than the fact that ', names.dorian,
    ...
    //p13
    p13as1a: pep.seq('Heart racing, ', names.jerry, ' sheathed ', pronouns.their(names.jerry0),
        ' dagger and looked out into the abandoned hallway. '),
    p13as1b: pep.seq(''),
    p13as1c: pep.seq(''),

    p13as2a: pep.seq('As if against ', pronouns.their(names.jerry0), ' own will, ', names.jerry,
    ...
    p13as4a: pep.seq(names.jerry, ' heard a click as the door shut and ', pronouns.their(names.jerry0),
    ...
    //p14
    p14as1a: pep.seq(names.jerry, ' pushed on the door, but it would not budge. '),
    p14as1b: pep.seq(''),
    p14as1c: pep.seq(''),
    ...
    p14as3a: pep.seq('How would ', names.jerry, ' explain this in the morning when someone found ',
    ...
    p15as2a: pep.seq(names.jerry, ' muttered as ', pronouns.they(names.jerry0),
        ' paced in front of the locked the door. '),
    p15as2b: pep.seq(''),
    p15as2c: pep.seq(''),

    p15as3a: pep.seq(html.i('o'), 'I was going home when I noticed the door open. '),
    p15as3b: pep.seq(''),
    p15as3c: pep.seq(''),
    ...
    //p16
    p16as1a: pep.seq(names.jerry, ' had a feeling that they would see through those flimsy excuses. '),
    p16as1b: pep.seq(''),
    p16as1c: pep.seq(''),

    p16as2a: pep.seq(names.jerry, ' knew why ', pronouns.they(names.jerry0), ' had closed the door. '),
    p16as2b: pep.seq(''),
    p16as2c: pep.seq(''),

    p16as3a: pep.seq('Even if ', pronouns.they(names.jerry0), ' didn\'t want to admit it. '),
    p16as3b: pep.seq(''),
    p16as3c: pep.seq(''),

    p16as4a: pep.seq(html.i('o'), 'Freedom from fighting is right through that portal. ', html.i('c'), ),
    p16as4b: pep.seq(''),
    p16as4c: pep.seq(''),

    p16as5a: pep.seq(names.jerry, ' heard the traitorous self-voice in the silence of the vault. '),
    p16as5b: pep.seq(''),
    p16as5c: pep.seq(''),

    p16as6a: pep.seq(pep.capitalize(pronouns.they(names.jerry0)),
        ' hadn\'t closed the door to prevent escapes. '),
    p16as6b: pep.seq(''),
    p16as6c: pep.seq(''),

    p16as7a: pep.seq('No one shut the door on ', pronouns.them(names.jerry0), '. '),
    p16as7b: pep.seq(''),
    p16as7c: pep.seq(''),

    p16as8a: pep.seq(pep.capitalize(pronouns.they(names.jerry0)),
    ...
    p17as2a: pep.seq('Try to spin a story about why ', pronouns.they(names.jerry0),
    ...
    p17as3a: pep.seq(pep.capitalize(pronouns.they(names.jerry0)),
        ' stepped away from the door and looked at the portal. '),
    p17as3b: pep.seq(''),
    p17as3c: pep.seq(''),

    p17as4a: pep.seq('It was now a blue-ish orange, and it seemed to be calling to ', pronouns.them(names
        .jerry0), '. '),
    p17as4b: pep.seq(''),
    p17as4c: pep.seq(''),

    p17as5a: pep.seq('Unlike many, ', pronouns.they(names.jerry0),
        ' had never actually been through the portal. '),
    p17as5b: pep.seq(''),
    p17as5c: pep.seq(''),

    p17as6a: pep.seq('By some small miracle, ', pronouns.they(names.jerry0),
        ' had missed most of the hazing that occurred in basic training. '),
    p17as6b: pep.seq(''),
    p17as6c: pep.seq(''),

    //p18
    p18as1a: pep.seq('But ', pronouns.they(names.jerry0), ' had heard the stories. '),
    p18as1b: pep.seq(''),
    p18as1c: pep.seq(''),

    p18as2a: pep.seq('How the mortal realm was much like the ', world.realm(names.realm),
    ...
    p18as4a: pep.seq('How there were no laws for those ', world.brethrens(names.realm),
    ...
    //p19
    p19as1a: pep.seq(html.i('o'), 'You can never come back, ', names.jerry, '. '),
    p19as1b: pep.seq(''),
    p19as1c: pep.seq(''),
    ...
    p19as3a: pep.seq('No more hanging out with ', names.reed, ' or bantering with ', names.dorian, '. '),
    p19as3b: pep.seq(''),
    p19as3c: pep.seq(''),

    p19as4a: pep.seq('You will be a traitor and a coward if you jump through that portal. ', html.i('c'), ),
    p19as4b: pep.seq(''),
    p19as4c: pep.seq(''),
    ...
    p20as2a: pep.seq(names.jerry, ' answered their silent musings out loud. '),
    p20as2b: pep.seq(''),
    p20as2c: pep.seq(''),
    ...
    p20as4a: pep.seq(names.kaden, ' has it out for me. '),
    p20as4b: pep.seq(''),
    p20as4c: pep.seq(''),
    ...
    //p21
    p21as1a: pep.seq(names.jerry, ' drew closer to the portal. '),
    p21as1b: pep.seq(''),
    p21as1c: pep.seq(''),
    ...
    p21as3a: pep.seq('It called to ', names.jerry, ', the promise of a new life awaiting. '),
    p21as3b: pep.seq(''),
    p21as3c: pep.seq(''),

    p21as4a: pep.seq('With nothing to lose, ', pronouns.they(names.jerry0), ' jumped. '),
    p21as4b: pep.seq(''),
    p21as4c: pep.seq(''),
...
let aBlock = {
    ...
    p16a: pep.seq(aPar.p16as1, aPar.p16as2, aPar.p16as3, aPar.p16as4, aPar.p16as5, aPar.p16as6, aPar.p16as7,
        aPar.p16as8, ),
